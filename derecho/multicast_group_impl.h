 ...
 * @param timeout_us The time that this node will wait for a sender in the group
...
MulticastGroup<dispatchersType>::MulticastGroup(
    ...
    std::vector<char> already_failed)
        : members(_members),
          ...
          sender_timeout(derecho_params.timeout_us),
          sst(_sst) {
...
std::function<void(persistence::message)> MulticastGroup<handlersType>::make_file_written_callback() {
    return [this](persistence::message m) {
        ...
        auto sequence_number = m.index * num_members + sender_rank;
        {
            ...
            sst->put((char*)std::addressof(sst->persisted_num[0]) - sst->getBaseAddress(),
    ...
    auto sst_receive_handler = [this](uint32_t sender_rank, uint64_t index_ignored, volatile char* data, uint32_t size) {
        ...
        if(new_seq_num > sst->seq_num[member_index]) {
            ...
            sst->put((char*)std::addressof(sst->seq_num[0]) - sst->getBaseAddress(),
                     sizeof(long long int));
            sst->put((char*)std::addressof(sst->nReceived[0][sender_rank]) - sst->getBaseAddress(),
                     sizeof(long long int));
        } else {
            sst->put((char*)std::addressof(sst->nReceived[0][sender_rank]) - sst->getBaseAddress(),
                     sizeof(long long int));
    ...
    auto stability_trig =
        [this](DerechoSST& sst) {
            ...
            if(min_seq_num > sst.stable_num[member_index]) {
                ...
                sst.put((char*)std::addressof(sst.stable_num[0]) - sst.getBaseAddress(),
                        sizeof(long long int));
            }
        };
    ...
    auto delivery_trig = [this](
        ...
        while(!locally_stable_messages.empty()) {
            ...
            if(least_undelivered_seq_num <= min_stable_num && !thread_shutdown && sst_multicast_group_created) {
                ...
                // }
                deliver_message(msg);
                // DERECHO_LOG(delivery_cnt, -1, "deliver_message() done");
        ...
        if(update_sst) {
            sst.put((char*)std::addressof(sst.delivered_num[0]) - sst.getBaseAddress(),
                    sizeof(long long int));
        }
    };
...
char* MulticastGroup<dispatchersType>::get_position(
    ...
    // DERECHO_LOG(position_cnt, -1, "provided a buffer");
...
void MulticastGroup<dispatchersType>::debug_print() {
    ...
    for(int i = 0; i < num_members; ++i) {
        for(int j = 0; j < num_members; ++j) {
            std::cout << sst->nReceived[i][j] << " " << std::endl;
        }
    }
    std::cout << std::endl;
}
